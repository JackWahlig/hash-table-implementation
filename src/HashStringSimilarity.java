...
public class HashStringSimilarity
{
	...
	public HashStringSimilarity(String s1, String s2, int sLength)
	{
		...
		alpha /= 31;
		String str = s1.substring(0, sLength);
		s1Table.add(new Tuple(key, str));
		//Get subsequent substrings and hashvalues based on first
		for(i = 1; i <= s1.length() - sLength; i++) {
			str = (s1.substring(i, i + sLength));
		...
		alpha /= 31;
		str = s2.substring(0, sLength);
		s2Table.add(new Tuple(key, str));
		//Get subsequent substrings and hashvalues based on first
		for(i = 1; i <= s2.length() - sLength; i++) {
			str = (s2.substring(i, i + sLength));
	...
	public float similarity()
	{
		...
		for(Tuple node : s1Table.getBucketArray()) {
			while(node != null) {
				sum += node.getFrequency() * (s2Table.search(node));
				node = node.getNext();
			}
		}
